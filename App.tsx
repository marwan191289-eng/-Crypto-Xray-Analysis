...
const App: React.FC = () => {
  ...
  const handleGlobalRefresh = useCallback(async () => {
    ...
    setNodeLatency(Math.floor(Math.random() * 20) + 80);
  ...
  const handleRefreshAI = useCallback(async () => {
    ...
    try {
      ...
      console.warn("AI Refresh Interrupted", e);
    } finally {
  ...
  return (
    <div className="h-screen bg-transparent text-text-bright flex overflow-hidden relative z-10">
      {/* SIDEBAR - Hidden on mobile, unified by top branding */}
      <aside className="w-[300px] bg-bg/70 border-r border-white/5 backdrop-blur-3xl hidden lg:flex flex-col p-8 z-50 flex-shrink-0">
        ...
        <div className="mt-10 relative p-6 rounded-[2rem] cursor-pointer group overflow-hidden border border-gold/10 hover:border-gold/30 transition-all duration-500 bg-gradient-to-b from-slate-900/50 to-black/50 reveal-anim" style={{ animationDelay: '0.8s' }} onClick={() => handleLinkClick(t.builtByMarwan)}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="bg-bg/40 backdrop-blur-3xl flex flex-col z-40 border-b border-white/5">
          {/* Top Bar: Always visible status */}
          <div className="bg-black/30 px-4 md:px-12 py-2 flex flex-row justify-between items-center border-b border-white/5 gap-2 overflow-x-auto no-scrollbar">
             <div className="flex items-center gap-4 md:gap-8 flex-nowrap whitespace-nowrap">
             ...
             <div className="flex items-center gap-6">
                <div className="flex items-center gap-2 whitespace-nowrap">
                   ...
                   <span className="text-[8px] md:text-[9px] font-black text-muted uppercase tracking-widest">Forensic Mainnet</span>
                </div>
             </div>
          </div>

          {/* Main Unified Header */}
          <div className="flex flex-col md:flex-row items-center justify-between px-4 md:px-12 py-4 md:py-6 gap-6">
            
            {/* Branding - Visible when Sidebar is hidden */}
            <div className="flex lg:hidden items-center gap-4 cursor-pointer group" onClick={() => scrollToSection('dashboard', dashboardRef)}>
              <div className="w-12 h-12 fusion-btn-xray rounded-xl flex items-center justify-center flex-shrink-0">
                <Eye className="w-6 h-6 text-white" />
              </div>
              <div className="text-start">
                <h1 className="text-xl md:text-2xl leading-none hologram-title">
                  {t.appNamePart1} <span className="text-accent">{t.appNameAccent}</span>{t.appNameSuffix}
                </h1>
              </div>
            </div>

            <div className="flex flex-row items-center justify-between md:justify-start gap-4 md:gap-10 flex-1 w-full md:w-auto">
              <div className="flex flex-col text-start min-w-0">
                 <span className="text-[8px] md:text-[9px] font-black text-accent uppercase tracking-[0.5em] mb-1 italic opacity-60 flex items-center gap-2">
                    <Target className="w-3 h-3" /> {t.targetIdentity}
                 </span>
                 <div className="flex items-center gap-4">
                   <div className="relative group overflow-hidden px-5 md:px-10 py-2.5 md:py-4 bg-white/5 rounded-xl md:rounded-2xl border border-white/10 text-lg md:text-2xl font-black text-white font-mono shadow-inner hover:border-accent/50 transition-all cursor-crosshair">
                     <div className="scanner-line"></div>
                     {selectedSymbol}/USDT
                   </div>
                   <div className="hidden sm:flex items-center gap-3 px-4 py-2 bg-success/10 rounded-xl border border-success/20">
            ...
            <div className="flex items-center gap-3 md:gap-6 flex-shrink-0 w-full md:w-auto justify-center md:justify-end">
              <button onClick={handleGlobalRefresh} disabled={isRefreshing} className="relative p-2.5 rounded-xl bg-slate-900/50 border border-white/10 hover:border-accent/50 hover:bg-accent/10 transition-all group overflow-hidden">
                <RefreshCw className={`w-4 h-4 text-slate-400 group-hover:text-accent transition-colors ${isRefreshing ? 'animate-spin' : ''}`} />
              </button>

              <div className="flex bg-black/50 rounded-xl border border-white/10 p-0.5">
                <button onClick={() => setLang('en')} className={`px-3 py-1.5 rounded-lg text-[9px] font-black transition-all ${lang === 'en' ? 'bg-accent text-white shadow-lg' : 'text-muted hover:text-white'}`}>EN</button>
                <button onClick={() => setLang('ar')} className={`px-3 py-1.5 rounded-lg text-[9px] font-black transition-all ${lang === 'ar' ? 'bg-accent text-white shadow-lg' : 'text-muted hover:text-white'}`}>AR</button>
              </div>

              <button onClick={handleRefreshAI} disabled={isAnalyzing || isMLRunning} className="fusion-btn-xray px-5 md:px-8 py-2.5 md:py-4 rounded-xl md:rounded-2xl text-white flex items-center gap-3 disabled:opacity-50 group transition-all shadow-glow shadow-accent/40">
                <Layers className={`w-4 h-4 md:w-5 md:h-5 ${isAnalyzing ? 'animate-spin' : 'group-hover:rotate-180 transition-transform'}`} />
                <span className="text-[10px] md:text-[13px] font-black uppercase tracking-[0.2em] whitespace-nowrap italic">{t.xRayAnalysis}</span>
              </button>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar relative" ref={dashboardRef}>
          <div className="p-4 md:p-14 space-y-12 md:space-y-20 pb-32 md:pb-24">
            ...
            <div className="flex flex-col gap-12 md:gap-24 w-full max-w-screen-2xl mx-auto min-w-0">
              <div ref={marketsRef} className="flex flex-col gap-6 md:gap-10 w-full reveal-anim" style={{ animationDelay: '0.2s' }}>
                <div className="cyber-card p-6 md:p-10 rounded-[2rem] md:rounded-[4rem] text-start border-white/10 shadow-3xl bg-bg/40 backdrop-blur-xl relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 md:gap-6 mb-6 md:mb-10 text-muted border-b border-white/5 pb-6 relative z-10">
                    ...
                    <div>
                      ...
                      <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-slate-500 mt-1 md:mt-2 block">Global Node Network</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 relative z-10">
                    {WATCHED_SYMBOLS.map(s => {
                      ...
                      const isUp = change >= 0;
                      return (
                        <button key={s} onClick={() => setSelectedSymbol(s)} className={`relative p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] border-2 transition-all duration-300 group overflow-hidden text-start ${selectedSymbol === s ? 'bg-accent/10 border-accent shadow-[0_0_30px_rgba(59,130,246,0.2)]' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
                          <div className="flex items-center justify-between mb-4 relative z-10">
                            <div className="flex items-center gap-3">
                               ...
                               <div>
                                  ...
                                  <span className="text-[8px] font-bold text-slate-500 tracking-wider">USDT</span>
                               </div>
                            </div>
                            <div className={`px-2 py-0.5 rounded-lg text-[8px] font-black border ${isUp ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                               {isUp ? '+' : ''}{change.toFixed(2)}%
                            </div>
                          </div>
                          <div className="relative z-10">
                             <span className="text-xl md:text-2xl font-black font-mono text-white tracking-tighter block">${price.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
          ...
          <footer className="footer-horizon pt-16 md:pt-32 pb-32 px-6 md:px-16 z-40 overflow-hidden">
             <div className="max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-20 mb-16 text-start">
                <div className="col-span-1 md:col-span-2">
                   <div className="flex items-center gap-6 mb-8 cursor-pointer group" onClick={() => scrollToSection('dashboard', dashboardRef)}>
                      <div className="w-16 h-16 fusion-btn-xray rounded-2xl flex items-center justify-center shadow-4xl">
                         <Eye className="w-8 h-8 text-white" />
                      </div>
                      <div>
                         <h2 className="text-3xl leading-none hologram-title">
                           {t.appNamePart1} <span className="text-accent">{t.appNameAccent}</span>{t.appNameSuffix}
                         </h2>
                         <div className="mt-4 flex items-center gap-3">
                            <span className="slogan-xray text-[11px]" data-text={t.seeInside}>{t.seeInside}</span>
                         </div>
                      </div>
                   </div>
                   <p className="text-[14px] text-muted leading-relaxed font-bold opacity-60 max-w-lg uppercase italic tracking-widest">{t.footerDesc}</p>
                </div>

                <div>
                   <h4 className="text-[13px] font-black text-white uppercase tracking-[0.5em] mb-8 italic flex items-center gap-4 border-b border-white/5 pb-6"><Terminal className="w-6 h-6 text-accent" /> {t.intelHub}</h4>
                ...
                <div>
                   <h4 className="text-[13px] font-black text-white uppercase tracking-[0.5em] mb-8 italic flex items-center gap-4 border-b border-white/5 pb-6"><Lock className="w-6 h-6 text-success" /> {t.securityProtocols}</h4>
             ...
             <div className="max-w-screen-2xl mx-auto pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-12">
                <div className="flex items-center gap-4">
                   ...
                   <span className="text-[10px] font-black text-muted uppercase tracking-[0.4em] italic">© 2025 BULLBEAREYE XRAY. {t.forensicNode}</span>
                </div>
                
                <div onClick={() => handleLinkClick(t.devContact)} className="group relative flex items-center gap-6 px-8 py-5 rounded-[3rem] cursor-pointer border border-gold/20 bg-slate-950/50 hover:bg-slate-900/80 hover:border-gold/50 transition-all duration-500">
                    <div className="relative p-3 bg-gradient-to-br from-gold/20 to-amber-900/20 rounded-2xl border border-gold/30">
                        <Award className="w-6 h-6 text-gold" />
                    ...
                    <div className="flex flex-col text-start relative z-10">
                        <span className="text-[8px] font-bold text-gold/60 uppercase tracking-[0.4em] mb-1">Developed By</span>
                        <span className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-gold to-white uppercase tracking-wider italic">
                            {t.builtByMarwan}
                        </span>
                        <span className="text-[8px] text-slate-400 font-bold uppercase tracking-[0.3em] mt-1 group-hover:text-gold transition-colors">
        ...
        {/* MOBILE NAVIGATION BAR - Mirroring Desktop Sidebar Icons */}
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-slate-950/95 backdrop-blur-2xl border-t border-white/10 p-2 z-50 flex justify-around items-center safe-area-pb shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
          {sidebarItems.slice(0, 5).map((item) => (
             <button key={item.id} onClick={() => scrollToSection(item.id, item.ref)} className={`flex flex-col items-center gap-1 p-3 rounded-2xl transition-all ${activeTab === item.id ? 'text-accent scale-110' : 'text-slate-500'}`}>
               <item.icon className={`w-5 h-5 ${activeTab === item.id ? 'drop-shadow-[0_0_8px_rgba(59,130,246,0.6)]' : ''}`} />
